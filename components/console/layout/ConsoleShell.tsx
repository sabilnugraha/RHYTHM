    ...
    <main className={`h-screen overflow-hidden ${shellTheme}`}>
      ...
      <div className="relative z-10 flex h-screen min-h-0">
        ...
        <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-2 pl-0 pt-0">
          ...
          <div className={`console-scrollbar ${isDarkMode ? 'console-scrollbar-dark' : ''} min-h-0 flex-1 overflow-auto rounded-b-xl rounded-tr-xl border-2 ${contentPadding} ${contentTheme}`}>{children}</div>
        </section>
      </div>
      <RhythmDock />
    </main>
