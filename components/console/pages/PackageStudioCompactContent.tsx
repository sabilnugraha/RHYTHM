...
type PackageItem = { id: string; name: string; domain: string; status: string; version: string; updated: string; s: number; o: number; c: number; ready: number };
...
export function PackageStudioCompactContent({ tab }: Props) {
  ...
  const placeholder = active.type === 'package' ? `Search ${activePackage.name}...` : active.type === 'new' ? 'Search new package...' : 'Search package...';
...
function IntroCard({ title, label, text, color, onClick }: { title: string; label: string; text: string; color: string; onClick?: () => void }) {
  ...
  return <Tag type={onClick ? 'button' : undefined} onClick={onClick} className={`rounded-lg border-2 border-black ${color} p-1.5 text-left shadow-[1.5px_1.5px_0px_#000]`}><p className="text-[8px] font-black uppercase tracking-[0.1em]">{label}</p><h2 className="mt-0.5 text-[9px] font-black leading-3 tracking-normal">{title}</h2><p className="mt-0.5 text-[8px] font-medium leading-3">{text}</p></Tag>;
}
...
function NewPackage() {
  return <section className="grid gap-1.5 lg:grid-cols-[minmax(0,1fr)_180px]"><main className="rounded-lg border-2 border-black bg-[#F6F1E8] p-2 shadow-[1.5px_1.5px_0px_#000]"><p className="text-[8px] font-black uppercase tracking-[0.12em] text-neutral-600">New Package</p><h2 className="mt-0.5 text-[9px] font-black leading-3 tracking-normal">Create Package</h2><div className="mt-2 grid gap-1.5 md:grid-cols-2">{['Name', 'Code', 'Domain', 'Owner'].map((label) => <label key={label}><span className="mb-0.5 block text-[8px] font-black uppercase">{label}</span><input className="h-6 w-full rounded-md border border-black bg-[#FFFDF8] px-1.5 text-[8px] font-semibold outline-none" placeholder={label} /></label>)}</div><label className="mt-1.5 block"><span className="mb-0.5 block text-[8px] font-black uppercase">Description</span><textarea className="min-h-16 w-full rounded-md border border-black bg-[#FFFDF8] px-1.5 py-1 text-[8px] font-semibold outline-none" placeholder="Package purpose" /></label><div className="mt-1.5 flex gap-1"><button className="rounded-md border border-black bg-[#FFE600] px-1.5 py-1 text-[8px] font-black">Save</button><button className="rounded-md border border-black bg-[#FFFDF8] px-1.5 py-1 text-[8px] font-black">Template</button></div></main><aside className="rounded-lg border-2 border-black bg-[#FFE600] p-1.5 shadow-[1.5px_1.5px_0px_#000]"><p className="text-[8px] font-black uppercase">Dirty</p><h3 className="text-[9px] font-black leading-3">New Package •</h3><p className="text-[8px] font-medium leading-3">Yellow dot = unsaved.</p></aside></section>;
}

function Builder({ pkg }: { pkg: PackageItem }) {
  return <section className="grid gap-1.5 lg:grid-cols-[145px_minmax(0,1fr)_180px]"><aside className="rounded-lg border-2 border-black bg-[#F6F1E8] p-1.5 shadow-[1.5px_1.5px_0px_#000]"><p className="text-[8px] font-black uppercase tracking-[0.12em] text-neutral-600">Map</p><h2 className="text-[9px] font-black leading-3 tracking-normal">{pkg.name}</h2><div className="mt-1.5 space-y-1">{steps.map((step) => <button key={step} className="w-full rounded-md border border-black bg-[#FFFDF8] p-1 text-left text-[8px] font-medium">{step}</button>)}</div></aside><main className="space-y-1.5"><Panel title={pkg.name} badge={pkg.status}><section className="grid gap-1.5 md:grid-cols-3"><LayerCard title="Structural" value={pkg.s} color="bg-[#00B9F2]" text="Tables, fields" /><LayerCard title="Operational" value={pkg.o} color="bg-[#9BFF00]" text="Menus, forms" /><LayerCard title="Control" value={pkg.c} color="bg-[#FF4E1F] text-white" text="Roles, audit" /></section></Panel><section className="grid gap-1.5 xl:grid-cols-[1fr_0.85fr]"><Panel title="Deployment" badge={`${pkg.s + pkg.o + pkg.c}`}><div className="grid gap-1.5 md:grid-cols-3"><LayerList title="Structure" color="bg-[#00B9F2]" items={['Objects', 'Fields']} /><LayerList title="Ops" color="bg-[#9BFF00]" items={['Menus', 'Forms']} /><LayerList title="Control" color="bg-[#FF4E1F] text-white" items={['Roles', 'Audit']} /></div></Panel><Panel title="Readiness" badge={`${pkg.ready}%`}><Readiness label="Struct" value={92} color="bg-[#9BFF00]" /><Readiness label="Ops" value={78} color="bg-[#00B9F2]" /><Readiness label="Ctrl" value={54} color="bg-[#FF4E1F]" /></Panel></section><Panel title="Components"><div className="grid gap-1">{components.map(([name, type, layer, deploy]) => <div key={name} className="grid gap-1 rounded-md border border-black bg-[#FFFDF8] p-1.5 text-[8px] font-medium md:grid-cols-[1fr_84px_56px_100px] md:items-center"><div><p className="text-[9px] font-black leading-3">{name}</p><p className="text-[8px] text-neutral-600">{type}</p></div><Pill className={layerColor(layer)}>{layer}</Pill><span className="text-neutral-600">Deploy</span><span className="font-black">{deploy}</span></div>)}</div></Panel></main><aside className="rounded-lg border-2 border-black bg-[#F6F1E8] p-1.5 shadow-[1.5px_1.5px_0px_#000]"><p className="text-[8px] font-black uppercase tracking-[0.12em] text-neutral-600">Inspector</p><h3 className="text-[9px] font-black leading-3">{pkg.name}</h3><p className="text-[8px] font-medium leading-3 text-neutral-700">Blueprint for {pkg.domain.toLowerCase()}.</p><div className="mt-1.5 space-y-1"><Info title="Visibility" text="Admin & end user" /><Info title="Install" text="Per client" /><Info title="AI Context" text="DDL metadata only" /></div></aside></section>;
}
...
function Info({ title, text, dark }: { title: string; text: string; dark?: boolean }) { return <div className={`rounded-md border border-black p-1.5 ${dark ? 'bg-[#111216] text-white' : 'bg-[#FFFDF8]'}`}><p className={`text-[8px] font-black ${dark ? 'text-white/70' : 'text-neutral-600'}`}>{title}</p><p className="text-[8px] font-medium leading-3">{text}</p></div>; }
