  ...
  return (
    <div className="mt-1 flex items-center gap-1 overflow-x-auto rounded-md border border-black/20 bg-[#F6F1E8]/70 px-1 py-0.5">
      {tabs.map((item) => {
        ...
        return (
          <button key={item.id} type="button" onClick={() => onSelect(item.id)} className={`flex h-[15px] shrink-0 items-center gap-0.5 rounded px-1 text-[7px] leading-[9px] tracking-normal transition-colors ${isActive ? 'bg-[#111216] font-medium text-white' : 'font-medium text-neutral-600 hover:bg-[#FFFDF8] hover:text-[#111216]'}`}>
            <span className="whitespace-nowrap">{item.title}</span>
            {item.dirty ? <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#FFE600]" /> : null}
            {!item.pinned ? (
              <span role="button" tabIndex={0} onClick={(event) => { event.stopPropagation(); onClose(item.id); }} className="grid h-[9px] w-[9px] shrink-0 place-items-center rounded-sm bg-[#E5484D] text-[6px] leading-none text-white">×</span>
            ) : null}
          </button>
        );
      })}
