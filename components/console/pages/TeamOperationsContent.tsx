    ...
    <aside className={`shrink-0 border-r-2 border-black bg-[#F6F1E8] ${collapsed ? 'w-10' : 'w-44'}`}>
      ...
      <div className="space-y-1 p-1.5">
        {menuGroups.map((group) => (
          <div key={group.id}>
            {!collapsed ? (
              <button className="mb-0.5 flex w-full items-center justify-between rounded px-1 py-0.5 text-[8px] font-black uppercase text-neutral-500" onClick={() => setOpen((current) => ({ ...current, [group.id]: !current[group.id] }))}>
                {group.id}<span>{open[group.id] ? '−' : '+'}</span>
              </button>
            ) : null}
            {(collapsed || open[group.id]) ? group.items.map((item) => (
              <button key={item.id} className={`mb-0.5 flex h-6 w-full items-center justify-between rounded-md px-1.5 text-left text-[8px] font-medium ${active === item.id ? 'bg-[#111216] text-white' : 'text-neutral-600 hover:bg-black/5'}`} onClick={() => setActive(item.id)} title={item.label}>
                <span className="truncate">{collapsed ? item.label.slice(0, 1) : item.label}</span>
                {!collapsed && item.badge ? <span className="rounded bg-[#FFE600] px-1 text-[7px] font-black text-black">{item.badge}</span> : null}
              </button>
            )) : null}
          </div>
        ))}
...
export function TeamOperationsContent({ tab }: Props) {
  ...
  return (
    <div className="flex min-h-full bg-[#FFFDF8] text-[#111216]">
      <TeamSidebar active={active} setActive={setActive} />
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 border-b-2 border-black bg-[#FFFDF8]/95 px-2 py-1 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-1.5">
            <div><p className="text-[8px] font-black uppercase tracking-[0.12em] text-neutral-500">RHYTHM Platform Console</p><h1 className="text-[10px] font-black leading-3">Team Operations</h1></div>
            <div className="flex items-center gap-1"><input className="h-6 w-44 rounded-md border border-black bg-[#FFFDF8] px-1.5 text-[8px] font-medium" placeholder="Search people, task, workspace..." /><SmallButton>+ Task</SmallButton><SmallButton dark>Invite</SmallButton></div>
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-1 text-[8px] font-medium">
            {['Overview', 'My Dashboard', 'Work Orchestration', 'Bottleneck Radar', 'People'].map((item) => <button key={item} className="h-[17px] rounded border border-black bg-[#F6F1E8] px-2">{item}</button>)}
            <span className="h-[17px] rounded border border-black bg-[#FF4E1F] px-2 py-0.5 text-white">No drag Kanban</span>
          </div>
        </header>
        <main className="p-1.5"><MainContent active={active} /></main>
      </div>
    </div>
  );
}
